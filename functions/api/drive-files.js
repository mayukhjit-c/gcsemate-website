...
export async function onRequest(context) {
    ...
    try {
        ...
        if (!folderId) {
            return json({ error: "Missing 'folderId' query param" }, 400);
        }
        ...
        driveUrl.searchParams.set(
            'fields',
            'files(id,name,mimeType,iconLink,webViewLink,webContentLink,hasThumbnail,thumbnailLink)'
        );
        ...
        if (!res.ok) {
            return json({ error: data.error?.message || 'Drive error' }, res.status);
        }
